...
class SerialControllerInterface:
    ...
    def update(self):
        ...
        if data == b'1':
            logging.info(f"KEYUP {self.mapping.button[data]}") #if (self.update_c%20==0) else print(end='')
            pyautogui.keyUp(self.mapping.button[data])
        else:
            logging.info(f"KEYDOWN {self.mapping.button[data]}") #if (self.update_c%20==0) else print(end='')
            pyautogui.keyDown(self.mapping.button[data])

        # fader
        valor = int.from_bytes(afec, 'big')
